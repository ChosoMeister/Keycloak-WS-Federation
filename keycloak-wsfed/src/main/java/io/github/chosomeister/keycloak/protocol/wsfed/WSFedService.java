...
import io.github.chosomeister.keycloak.protocol.wsfed.builders.WSFedProtocolParameters;
...
public class WSFedService extends AuthorizationEndpointBase {
     ...
     * @param event
     */
    public WSFedService(KeycloakSession session, EventBuilder event) {
        super(session, event);
    }
    ...
    public Response handleWsFedRequest(boolean redirectToAuthentication) {
        ...
        if (httpRequest.getHttpMethod().equals(HttpMethod.POST)) {
            requestParams = httpRequest.getDecodedFormParameters();
        } else {
