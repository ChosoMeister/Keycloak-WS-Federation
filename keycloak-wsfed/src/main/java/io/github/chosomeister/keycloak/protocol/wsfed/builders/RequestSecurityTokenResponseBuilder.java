...
import io.github.chosomeister.keycloak.common.wsfed.writers.WSTrustResponseWriter;
...
public class RequestSecurityTokenResponseBuilder extends WSFedResponseBuilder {
    ...
    public RequestSecurityTokenResponse build() throws ConfigurationException, ProcessingException {
        ...
        if(samlToken != null) {
            ...
            bstt.setValue(java.util.Base64.getEncoder().encodeToString(jwt.getBytes(java.nio.charset.StandardCharsets.UTF_8)));
    ...
    public Document encryptDocument(Document samlDocument) throws ProcessingException {
        try {
            ...
            Element assertion = samlDocument.getDocumentElement();
            QName assertionName = new QName(assertion.getNamespaceURI(), assertion.getLocalName());
            QName encryptedAssertionName = new QName(assertion.getNamespaceURI(), "EncryptedAssertion", "saml");
            XMLEncryptionUtil.encryptElement(assertionName, samlDocument, encryptionPublicKey,
                    secretKey, encryptionKeySize, encryptedAssertionName, true);
