...
import io.github.chosomeister.keycloak.common.wsfed.utils.WSFedValidator;
...
public class WSFedEndpoint {
    ...
    private static final int MAX_CONTEXT_LENGTH = 8192;
    ...
    protected Response execute(String wsfedAction, String wsfedResult, String context) {
        ...
        if ((wsfedResult != null && wsfedResult.getBytes(StandardCharsets.UTF_8).length > MAX_RESPONSE_BYTES)
                || (context != null && context.length() > MAX_CONTEXT_LENGTH)) {
            event.event(EventType.IDENTITY_PROVIDER_RESPONSE);
            event.error(Errors.INVALID_REQUEST);
            return ErrorPage.error(session, null, Response.Status.REQUEST_ENTITY_TOO_LARGE, Messages.INVALID_REQUEST);
        }
    ...
    protected Response handleSignoutRequest(String context) {
        ...
        List<UserSessionModel> userSessions = session.sessions()
                .getUserSessionByBrokerUserIdStream(realm, result.getSession().getBrokerUserId())
                .toList();
    ...
    protected Response handleLoginResponse(String wsfedResponse, RequestedToken token, String context) {
        try {
            BrokeredIdentityContext identity = new BrokeredIdentityContext(token.getId(), config);
            if (context != null) {
        ...
        if (decodedContext.contains("redirectUri=")) {
            ...
            if (decodedContext.contains("&code=")) {
                ...
                if (clientCode != null && clientCode.getCode().isValid(CommonClientSessionModel.Action.AUTHENTICATE.name(), ClientSessionCode.ActionType.LOGIN)) {
                    ...
                    identity.setAuthenticationSession(clientCode.getClientSession());
            ...
            identity.setAuthenticationSession(callback.getAndVerifyAuthenticationSession(decodedContext));
        }
        return null;
    }
    ...
    protected RequestSecurityTokenResponse getWsfedToken(String wsfedResponse) throws ParsingException, IOException {
        ...
        byte[] responseBytes = wsfedResponse.getBytes(StandardCharsets.UTF_8);
        if (responseBytes.length > MAX_RESPONSE_BYTES) {
            throw new ParsingException("WS-Federation response exceeds the configured safety limit");
        }
