...
import io.github.chosomeister.keycloak.protocol.wsfed.mappers.WSFedSAMLAttributeStatementMapper;
import io.github.chosomeister.keycloak.protocol.wsfed.mappers.WSFedSAMLRoleListMapper;
...
//        String nameId = getNameId(nameIdFormat, clientSession, userSession);
        ...
        builder.issuer(responseIssuer)
                .assertionExpiration(effectiveAssertionLifespan(realm, clientSession.getClient(), userSession) > 0
                        ? effectiveAssertionLifespan(realm, clientSession.getClient(), userSession)
        ...
        Set<ProtocolMapperModel> mappings = org.keycloak.services.util.DefaultClientSessionContext
                .fromClientSessionScopeParameter(clientSession, session)
                .getProtocolMappersStream().collect(java.util.stream.Collectors.toSet());
